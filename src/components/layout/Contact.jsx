    ...
    <section ref={ref} className='mb-[100px]'>
        <Container>
            ...
            <div className='flex  lg:justify-start'>
                    <h2 className='font-mont mx-5 lg-mx-0  font-bold text-[#494949] text-2xl'>{title}</h2>
                </div>
            ...
            <div className='pt-6 pb-1 flex lg:text-left  lg:justify-start  mx-5 lg-mx-0'>
                <p className='font-pop font-normal text-sm text-[#888888] w-auto lg:w-[323px] leading-6'>{subtitle}</p>
            </div>
           ...
           <div className='w-[50%] contact_res:mt-4 lg:mt-0 mx-5 lg-mx-0 '>
            ...
            <div  className='w-[50%]  lg:flex flex-wrap lg:justify-evenly mx-5 lg-mx-0  lg:mt-[-120px]'>
                            ...
                            <form onSubmit={onSubmit}>
                                <input onChange={(e)=>setname(e.target.value)}  className='mt-8 contact_res:mt-0 lg:mt-8 lg:mt-4 px-3 py-2 w-[240px] border-[1px]  border-transparent border-b-[#D9D9D9] bg-white border shadow-sm  placeholder-slate-400 focus:outline-none focus:ring-b-sky-500  rounded-md sm:text-sm
                                                placeholder:font-pop  placeholder:text-sm  placeholder:text-[#B3B3B3' type="text" placeholder='Your Name' name="name"/>
                                <input onChange={(e)=>setemail(e.target.value)} className='mt-4 px-3 py-2 w-[240px] border-[1px]  border-transparent border-b-[#D9D9D9] bg-white border shadow-sm  placeholder-slate-400 focus:outline-none focus:ring-b-sky-500  rounded-md sm:text-sm
                                                placeholder:font-pop  placeholder:text-sm  placeholder:text-[#B3B3B3 lg:ml-4' placeholder='Your Email' type="email" name="email"/>
                                <textarea onChange={(e)=>setmessage(e.target.value)} className='resize mt-[25px] lg:mt-[50px] rounded-md w-[250px] lg:w-[500px]  border-[1px] h-[148px] border-[#D9D9D9] bg-white border shadow-sm  placeholder-slate-400 focus:outline-none focus:ring-b-sky-500  rounded-md sm:text-sm  placeholder:font-pop  placeholder:text-sm  placeholder:text-[#B3B3B3] placeholder:p-4' placeholder='Your Message' name="message"></textarea>
                               <div className='lg:flex'>
                                  <button className='font-pop text-[#FFFFFF] px-6 py-5 contact_res:py-5 mt-[25px] lg:mt-[44px] contact_res:px-12 bg-[#393939] rounded-md
                                    text-[11px] contact_res:text-[11px]  md:text-sm font-light  hover:bg-slate-600  hover:text-[#f5f5f5] duration-300' type="submit">
                                      {btn}
                                </button>
                                <div className='mt-2 lg:mt-14'>
                                  <span className='lg:ml-12 font-mont text-sm lg:text-base text-black'>
                                    {nameErr?nameErr:emailErr?emailErr:messageErr?messageErr:msgsucc?msgsucc:""}</span>
